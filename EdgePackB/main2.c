...
repeatGame:
		...
		do {
			...
			currentEdge = (unsigned char *) &(EDGE[currentMap*64]);
			tmpw = currentMap*3;
			...
			do{
				...
				if( a != 0 ){
					...
					if((a==  'L' || a=='F'|| a==9) && edgey<Edgesize-1 ) {
						if( edgez <7
		...
		printf("\nNIVEAUX : %d - SCORE FINAL : %d\nREBOOT TO LEAVE OR A KEY = DO IT AGAIN",currentMap+1,score);
